...
第八轮把搜索布局换成经严格网格证书（1 m 网格，最小余量1.73 m）认证的22站列表，删除 `config.json` 中的 `q4_station_list` 即恢复25站。新种子配对：随机集494.5→478.4秒/源、边界向外588.7→557.9、全定向606.1→574.9，220例全部清除。同轮为问题3加入圆心补测（`q3_refine_max_m=300`），100例留出262.6→260.0秒/源，失败清除3.00→0.38次/源。详见 [布局搜索报告](q4_layout_search.md)。
