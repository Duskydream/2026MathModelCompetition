...
\begin{figure}[!htbp]
  ...
  \includegraphics[width=0.6\linewidth]{figures/fig-q2-region.png}
...
\cref{thm:twobearing}有三个直接后果。\textbf{其一，光学覆盖兜底不可省略}：任何“两次测向再清除”的流水线都不完备，必须保留对可行域有连续覆盖证明的清除点列。\textbf{其二，不宜盲目追求正交交会}：远源反正要进入光学覆盖，为降低 $\mecr$ 而多走数百米往往不划算。\textbf{其三，近源应优先获得第二次测向}：由一阶估计 $\mecr\propto r_1r_2$，对近源两次测向常常够用——以 $(200,100)$ 为第二点时，首测距离在 $380\mmet$ 以内基本可一次认证清除（\cref{tab:q2-need}）。
