...
\begin{proof}
因 $\rho\ge\max(\rho_{\min},r)$，只需证 $\|q-g\|\le\max(\rho_{\min},r)$。当 $r\le\rho_{\min}$ 时，$\|q-r\uvec\delta\|^2$ 关于 $r$ 是凸二次函数，在 $[0,\rho_{\min}]$ 上的最大值在端点取得，两端分别给出\cref{eq:csafe}的两式（其中 $q^{\!\top}\uvec{\delta}=a\cos\delta+b\sin\delta\ge a\cos\varepsilon-|b|\sin\varepsilon$）；当 $r>\rho_{\min}$ 时只需证 $\|q-g\|\le r$，其右端关于 $r$ 单调增，最紧情形仍为 $r=\rho_{\min}$。又第二式等价于 $\|q-\rho_{\min}\uvec{\pm\varepsilon}\|\le\rho_{\min}$，故得\cref{eq:lens}。
\end{proof}
...
\paragraph{关于候选区域的三点说明.} 其一，$\mathcal C_{\rm safe}$ 是\emph{充分}而非充要条件，实际可用点可能更多，但那依赖未知的 $r$ 与 $\rho$，不能作为策略依据。其二，它允许 $q$ 落在目标圆之外（附件允许）。其三，原点 $q=\bm0$ 虽满足约束但必须排除——同点重复检测不提供新约束，且交会角与点线距离公式在该点同时退化为零；实现中要求 $\|q-s_1\|\ge q_{\min}=100\mmet$，该门槛与共享测站规则复用。
...
进一步考察两次测向是否足以保证认证清除：既然第二次测向能把区域缩小，那么再挑得讲究一些，是不是就能把 $\mecr$ 压进 $20\mmet$，从而省掉光学覆盖？答案是否定的，而且这一点可以\emph{严格}地证明——不依赖任何近似公式。
...
\paragraph{单目标最优不等于整局最优.} 正式方案取较短的 $(200,\pm100)$ 而非单目标更优的 $(750,450)$：远源反正要走光学覆盖，多走 $650\mmet$（$130\msec$）换来的扫描节省有限；多目标下同一次移动可服务多个频道（\cref{sec:q3:shared}）；走远还会带来后续折返。固定种子的配对实验也显示自适应选点无改善（见附录~\ref{app:q2}），故保留固定 $(200,\pm100)$，把“是否再测一次”交给\cref{sec:q3:refine}的补测精化与\cref{sec:q4:online}的扫描成本门控。

\paragraph{本问小结.} 保证接收条件、严格分辨反例与精确枚举三者的结论一致：\textbf{两次测向不能保证一次认证清除，光学覆盖兜底不可省略}。据此，正式方案取固定的 $(200,\pm100)$，把“是否再测一次”的判断交给在线层的补测精化与扫描成本门控。
