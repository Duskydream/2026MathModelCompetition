...
\paragraph{工程决策：$22$ 站与 $25$ 站的取舍.} 综合以上，本文以 $22$ 站为默认：它在题设参数下通过严格连续域证书，且巡站路径更短；$25$ 站解析布局作为保守备用保留。程序在启动时用 $20\mmet$ 粗网格复核，若当前参数下证书不满足（例如接收半径下界被调低到 $990\mmet$，$22$ 站约 $6\sim10\mmet$ 的余量消失），则\textbf{拒绝使用 $22$ 站并回退到 $25$ 站}。题目明确允许机器狗走出目标区域，故外环站与区域外第二测点都合规，且与 $2000000\mmet$ 的坐标上限相距甚远。
...
\cref{tab:q4holdout}显示 $22$ 站在五类场景的均值与 $\mathrm{P}_{95}$ 上都优于 $25$ 站，最困难的“边界向外”“全部定向”改善最明显（$30.8$ 与 $31.2\msec/$源）；但\textbf{逐例并非全面占优}，个别案例因发现顺序改变而变慢。本文保留的是“均值与分位数改善、清除率保持 $100\%$”这一较弱但站得住的结论（典型局轨迹与逐例配对见\cref{fig:q4-track}）。
...
把巡站、全站扫描与到达清除三项相加，得到第四问的成本基准；按源数分层，官方演练比该基准高 $6\%\sim11\%$。需要强调\textbf{这是经验构造而非可证下界}，只能说明“现有方案已接近一条合理的成本底线”。若放弃覆盖保证，用 $40$ 万个随机定向源做蒙特卡洛估计：$16$ 站时每局期望漏检 $0.14$ 个，约七局就有一局清除率低于 $100\%$；按“清除率是硬约束”的准则，这些方案一律不采用。完整的基准公式、分层表与风险表见附录~\ref{app:fixed}。

\paragraph{本问小结.} 采用的 $22$ 站布局同时通过 $h=1\mmet$ 细网格与 $20\mmet$ 粗网格两次凸包检查，$220$ 例留出验证与 $70$ 局官方演练均全部清除。需要强调：\textbf{$22$ 是本文找到并认证过的最小站数，而非对任意布局的理论下界}。
