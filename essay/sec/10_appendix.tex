...
% 附录的节编号为 A、B、…，标题不用顿号；子节编号改用 \thesection（A.1 而非 7.1）
...
\begin{table}[!htbp]
  ...
  \begin{tabularx}{\textwidth}{>{\raggedright\arraybackslash}p{4.6cm} L}
    ...
    \texttt{official\_practice/} & 演练样本：\texttt{q3\_runs.csv}、\texttt{q4\_runs.csv} 为第三、四问各 $70$ 局的逐局明细，
      \texttt{official\_runs.json} 另含被排除记录的清单与原因 \\
    \texttt{支撑材料/formal-p*.jlog} & 第三、四问各三次正式测试的加密行为日志，文件名未作任何修改 \\
