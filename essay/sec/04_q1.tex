...
\paragraph{复杂度.} 判空与判无界共 $5$ 次二维线性规划；顶点枚举为 $O(m^2)$ 对交点、每个交点 $O(m)$ 次可行性检验，合计 $O(m^3)$；直径枚举 $O(k^2)$。本题单个源的检测点很少（$K\le4$），该实现足够；若 $K$ 很大，可改用半平面交（$O(m\log m)$）加旋转卡壳（$O(k)$），总复杂度降至 $O(m\log m)$。

\paragraph{四类退化的处理.} $P$ 为空表示读数与误差界不相容（例如误差界设小了），此时必须报错并排查角度约定、取整或坐标系，\emph{不能}静默地取某个近似交点；$P$ 无界发生在所有楔形的方向锥未能互相封闭时（典型情形是只有一个检测点，或多个检测点近似共线且同侧），此时直径为 $+\infty$，用一个人为的大方框去截断再报一个有限直径是错误的；$P$ 为线段或单点时，\cref{alg:diam}给出的 $D$ 自然退化，无需特判。
...
\begin{figure}[!htbp]
  \centering
  \fbox{\parbox[c][6cm][c]{0.9\textwidth}{\centering\textbf{【此处插图】图~\thefigure：问题一的反例与覆盖性示意}\\[4pt]
  \small 画法建议：左子图画\cref{exa:tri}的三个检测点（远处，可用折断坐标轴或局部放大框）及三条示向度中心线与各自 $\pm1^\circ$ 的虚线，
...
\paragraph{什么时候可以覆盖：两站交会的情形.} 需要强调的是，题目附图所示的\emph{两站}交会四边形，当两站到源的距离远大于区域尺度时趋于平行四边形，此时总能被同直径圆覆盖。设两条楔形的中心线以交会角 $\psi$ 相交，则 $P$ 是一个由四条直线围成的“细长四边形”。对平行四边形而言，长对角线两端是两个锐角顶点，另外两个顶点的内角为 $180^\circ-\psi\ (\ge90^\circ)$，因而落在以长对角线为直径的圆内，即 $\mecr=D/2$。\cref{tab:q1-example}给出两个数值算例，$\mecr$ 与 $D/2$ 在四位小数下完全相同，验证了这一点。
