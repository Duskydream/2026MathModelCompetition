...
\begin{abstract}
本题的核心并非单纯定位，而是在源数未知、测向误差有界、辐射方向未知的条件下，实现\textbf{可证明全覆盖、可认证清除、可执行调度}的闭环搜索系统。本文把机器狗在圆形目标区域内自主完成“发现—定位—清除”的任务建模为\textbf{部分可观测环境下、带覆盖保证的在线搜索--定位--清除问题}，按“\emph{覆盖保证} $\to$ \emph{布局下界} $\to$ \emph{在线调度} $\to$ \emph{有界误差定位与认证清除}”四层展开，并始终以\textbf{清除率 100\% 为硬约束}，只在其满足时才比较时间。

针对\textbf{问题一}，把每个示向度读数化为两条线性半平面约束，定位区域即有限个\emph{前向楔形}之交；给出“判空与判无界 $\to$ 边界线求交过滤 $\to$ 极角排序 $\to$ 顶点对枚举”的直径算法，并由 Jung 定理证明以该直径为直径的圆一般\emph{不能}覆盖该区域（三站算例 $D=40\mmet$、最小包围圆 $23.094\mmet>20\mmet$）。由此确立全文判据：\textbf{$\mecr\le20\mmet$ 才可一次认证清除}，而非 $D\le40\mmet$。

针对\textbf{问题二}，导出一个\textbf{不依赖未知源距与接收半径}的保证接收区域 $\mathcal{C}_{\rm safe}$（三个半径 $1000\mmet$ 圆盘之交），并证明交会角下界只由横向偏移决定、与前进距离无关；进一步给出严格分辨极限——存在相距 $41\mmet$ 的两源产生完全相同的两次观测，故仅凭两次测向无法保证一次清除，光学覆盖兜底不可省略。

针对\textbf{问题三}，证明“圆心加正六边形”七站布局的最坏覆盖距离 $988.5114\mmet<1000\mmet$，并由 Bezdek 界证明\textbf{七站是保证发现的最小站数}；配合有界误差可行域递推、\texttt{no\_signal} 圆盘排除、最小包围圆认证清除与混合贪心调度，官方演练 $70$ 局、$917$ 个源\textbf{全部清除}，平均 $255.6\msec/$源。

针对\textbf{问题四}，给出方向覆盖的\textbf{充要条件}：源 $g$ 对任意朝向都能被发现，当且仅当 $g$ 落在其 $\rho$-近邻测站的凸包内。据此把测站由 $49\to27\to25$ 压到 \textbf{22 站}，并用 $h=1\mmet$ 网格证书（$1018.68$ 万点，最小余量 $1.7331\mmet$）完成连续域严格验证；在所考察布局族内 $21$ 站及以下均无法通过同一证书。官方演练 $70$ 局、$887$ 个源全部清除，平均 $511.2\msec/$源。

$140$ 局演练还表明\textbf{整局总时间几乎完全由与源数无关的固定成本决定}，平均定位清除时间随源数下降主要是\emph{分母效应}。综上，本文把每一条结论都落成可执行的启动校验、逐例不变量检查与失败留档；三次正式测试（第三、四问各三次）全部清除，合并平均 $239.0$ 与 $498.8\msec/$源。
