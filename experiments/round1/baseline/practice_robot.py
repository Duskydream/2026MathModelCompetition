...
def main(argv=None):
    ...
    meta=dict(question=args.question,strategy=args.strategy,declared_mode='practice',mode_verified_by_protocol=False,case_code=args.case_code,config=cfg,python=sys.version,code_sha256={name:hashlib.sha256((base/name).read_bytes()).hexdigest() for name in ['practice_robot.py','model.py','optimized.py','config.json']})
