...
演练模块要在模拟器里手动选择，协议本身区分不了“演练/正式”会话。程序严格
按附件 2 串行发送、逐次等待响应；网络中断后用原请求内容与原 request_id
重试，结果未知的动作标记为 uncertain 并禁止再发不同请求，避免重复计时或
状态错位。每次发送前先写日志，防止中断掩盖一个可能已执行的动作；关键源码
的 sha256 与配置一起写入 run_config.json，用于事后筛选同版本成绩。
"""
import argparse
import hashlib
import http.client
...
class UncertainAction(RuntimeError):
    """请求可能已经执行过：禁止再发送不同的下一个动作。"""
    pass

class PracticeHTTP:
    """官方 HTTP 协议的客户端：请求计数、重试、虚拟时间对账与动作日志。"""
    ...
    def exit(self):
        """主动退出会话，要求 exit_reason 为 user_exit。"""
        r=self.post('/exit')
...
def main(argv=None):
    """运行一次演练，并落盘 summary、run_config 与定位区域记录。"""
