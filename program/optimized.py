"""路径排序与共享测站：只让可观测的响应进入决策。

搜索任务与目标任务放进同一循环，每步按“谁更近”决定先做哪一个，减少基线
逐站扫描带来的折返。第二测点取 (200, ±100)：远源终归要进入光学覆盖，把
第二点放近一些即可，继续外移换来的扫描节省有限。共享测站只比较一步移动
代价，属于纯启发式；试过的两步前瞻会把总移动距离拉长，故保留一步贪心。
variant='legacy' 保留旧路径，用于配对消融。
"""
import numpy as np
...
class OptimizedPolicy(Policy):
    ...
    def __init__(self,transport,cfg,question,variant='shared'):
        ...
        # 第四问默认 mixed_ring：选择性重测 + 有界等待 + 认证条带覆盖。
    ...
    def destination(self,ch):
        ...
        # legacy 路径：沿方位方向前移 650 m、再左右各偏 250 m 取较近的一侧。
    ...
    def shared_scan(self,exclude=None):
        # 到达一个测点/清除点后，顺带看其他待处理频道是否也值得测一次。
        # TODO: 这里只比较一步移动代价，两步前瞻尚未纳入调度。
