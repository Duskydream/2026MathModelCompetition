"""第三问专用：共享测站下的混合路由，以及对观测的保守约束。

第三问的源都是全向源，“在某站收不到”蕴含“距离超过 ρmin”，可以对该站
邻域做圆盘排除；第四问的源可能定向，这条信息用不上。圆盘排除的边界点处理
过一次证书失效（浮点把边界交点误判到圆盘内），此后保留边界顶点、对接近
端点的根做夹取，并补了回归测试。可行域落在 20~300 m 时先到圆心补测精化，
明显减少了后续的失败清除。
"""
import math
import numpy as np
...
def outside_disk_hull(poly,center,radius):
    """凸多边形“挖去”一个开圆盘后的凸包外近似。

    极值点只可能来自圆盘外的多边形顶点或边与圆的交点；凸包取外近似，
    宁可多留一小块已排除区域，也要保证真源一定还在区域内，否则包含性
    不变量会被破坏。数值退化时（点数不足、凸包失败）直接返回输入多边形，
    同样保持保守。
    """
    points=[]
...
class Q3Planner:
    ...
    def __init__(self,policy):
        ...
        # 对不超过该半径的可行域，先到圆心（必要时再横向偏移）补测一次，
        # 仍不够才退回光学网格。
    ...
    def constrain(self,ch):
        ...
        # 支撑半平面取外切：圆边界附近的一圈真实位置必须保留，内接会把它们切掉。
        ...
        for station in self.silent[ch]:
            # 第三问均为全向源且接收半径 ≥ R_min；排除盘略取小一点留浮点余量。
    ...
    def destination(self,ch):
        ...
        return point,True
    def refine(self,ch):
        """对 20~300 m 的可行域用一到两次近距离测向收缩；认证清除仍然精确。"""
        p=self.p;t=p.tracks[ch]
